...
def tainjin():
    url = "http://jyxy.tju.edu.cn/en/Faculty.html"   # homepage url
    ...
    filename = "tainjin.txt"
    f = open(filename, "w")

    excel_filename = "tainjin.csv"
    ...
    d1 = soup.find_all("div" ,{'class':'TeContentlist'})
    dd = []
    for i in d1:
        d = i.find_all('li')
        dd.extend(d)
    ...
    for i in dd:
        ...
        name = a.find('p' , {'class':'teachername'}).get_text()               # extracting prof name
        email =  a.find('p' , {'class':'teacheremail'}).get_text()
